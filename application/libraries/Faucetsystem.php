...
class Faucetsystem {
    ...
    public function __construct( $currency = "BTC", $connection_options = null) {
    ...
    public function __execPHP($url, $params = array()) {
        ...
        if($this->options['local_cafile']) {
            $opts["ssl"]["cafile"] = R . '/storage/cacert.pem';
        }
    ...
    public function send($to, $amount, $referral = "false", $ip = "127.0.0.0") {
        $r = $this->__exec("send", array("to" => $to, "amount" => $amount, "referral" => $referral, "ip" => $ip));
